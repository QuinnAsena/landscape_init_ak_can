...
landcapes <- lapply(landscape_dirs, \(ind) {
  ...
  out_sup <- file.path(ind, "supporting_data", "gis")
  dir.create(out_sup, recursive = TRUE, showWarnings = FALSE)
  dir.create(out, recursive = TRUE, showWarnings = FALSE)

  above_lc <- list.files(file.path(ind, "supporting_data", "ABoVE_LandCover"), full.names = TRUE)
  ...
  crs(r2) <- crs(r)
  writeRaster(r2, file.path(out_sup, "env.grid_disagg_10.tif"), overwrite = TRUE, datatype = "INT4S")
})
