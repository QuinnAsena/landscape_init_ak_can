...
# BATCH is the I/O dial, and it is the number of LINES in a cmdfile -- not --steps-per-node.
# launch_cf submits one subjob per node and PBS can start them all at once, so every line in
# a cmdfile can be running at the same moment. Each line forks 9 workers for scenario (5 for
# spinup), and every worker opens its OWN sqlite connection and writes its own parquet. So a
# whole landscape in one file (156 lines) would be ~1400 concurrent readers/writers on
# scratch, against the 90 that job 7349725 actually proved (18 lines x 5 workers, one node).
# 3 nodes keeps it near 320. --steps-per-node only decides how many steps share a node, i.e.
# memory and cores per node. Same chunking pattern as scenario-launch/generate_cmdfiles.sh.
NODES_PER_JOB=3
BATCH=$(( NODES_PER_JOB * spn ))      # 36 lines for scenario (spn 12), 54 for spinup (spn 18)

total=0
for n in ${landscapes}; do
    landscape="landscape_alaska_${n}_${period}"

    # Build every command line for this landscape first, then chunk into batches of BATCH.
    lines=()
    while IFS=$'\t' read -r treatment reps; do
        for rep in $(seq 1 "${reps}"); do
            lines+=("${PRE} ${DIR}/analysis-scripts/process_${analysis}.R \"${landscape}\" \"${treatment}\" ${rep}")
        done
    done < "${tmp_treat}"
    ...
    # Clear THIS landscape's old files only, never the whole directory, so regenerating one
    # landscape later (06, once its runs finish) cannot delete another landscape's batches.
    # The un-suffixed name is the pre-2026-09-15 layout of one file per landscape.
    rm -f "${out_dir}/cmdfile_${analysis}_${kind}_${n}.sh" \
          "${out_dir}"/cmdfile_${analysis}_${kind}_${n}_b*.sh

    for ((b=0; b<n_batches; b++)); do
        f=$(printf "%s/cmdfile_%s_%s_%s_b%02d.sh" "$out_dir" "$analysis" "$kind" "$n" $((b+1)))
        {
            ...
            echo "#"
            echo "# ${analysis} processing for ${landscape} -- batch $((b+1)) of ${n_batches}."
            ...
            printf '%s\n' "${size_note[@]}"
            echo "#"
            echo "# Submit the whole set as a chain, one batch at a time:"
            echo "#   bash analysis-scripts/submit_chain_process.sh ${analysis} ${kind}"
            echo "#"
        ...
        printf "  %-38s %4d lines, 0 blanks -> %2d node(s) at --steps-per-node %d" \
